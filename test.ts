...
test("do not get nested urls from query strings", t => {
  const text =
    "You can read http://www.awin1.com/cread.php?a=b&p=https%3A%2F%2Fuk.hotels.com%2Fhotel%2Fdetails.html%3Ftab%3Ddescription%26hotelId%3D287452%26q-localised-check-in%3D15%2F12%2F2017%26q-localised-check-out%3D19%2F12%2F2017%26q-room-0-adults%3D2%26q-room-0-children%3D0%26locale%3Den_GB%26pos%3DHCOM_UK for more info";
...
test("exclude matching urls", t => {
  const text = `${fs.readFileSync(
    "fixture.txt",
    "utf8",
  )} http://w3.org/2000/svg, http://foobar.com/document.html, https://www.w3schools.com/`;
...
test("get urls without scheme", t => {
  const text =
    "Lorem ipsum dolor sit amet, //sindresorhus.com consectetuer adipiscing http://yeoman.io elit. www.github.com";
...
test("get schemeless url from query string", t => {
  const text =
    "You can read http://www.awin1.com/cread.php?a=b&p=%2F%2Fuk.hotels.com%2Fhotel%2Fdetails.html%3Ftab%3Ddescription%26hotelId%3D287452%26q-localised-check-in%3D15%2F12%2F2017%26q-localised-check-out%3D19%2F12%2F2017%26q-room-0-adults%3D2%26q-room-0-children%3D0%26locale%3Den_GB%26pos%3DHCOM_UK for more info";
...
test("handles parens", t => {
  ...
  t.deepEqual(getUrls(text), new Set(["https://sindresorhus.com/some/example"]));
});

test("handles Markdown", t => {
  const text =
    "foo [![](https://sindresorhus.com/unicorn.png)](https://sindresorhus.com/?foo=bar) foo";

  t.deepEqual(
    getUrls(text),
    new Set(["https://sindresorhus.com/unicorn.png", "https://sindresorhus.com/?foo=bar"]),
  );
});
